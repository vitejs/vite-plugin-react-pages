...
> Fun fact: vite-pages itself doesn't render any concrete DOM node. All DOM nodes are rendered by <Link to="/theme-customization">theme</Link>.
...
Most themes(.e.g <Link to="/official-theme">the official theme</Link>) ask users to `export default` a React component from each page file. But that is not a requirement from vite-pages core. We will talk about this later.
...
As stated in <Link to="/theme-customization">the theme customization doc</Link>, a theme is a React component. It will be rendered by vite-pages core, and get useful info from props:
...
Most themes(.e.g <Link to="/official-theme">the official theme</Link>) ask users to `export default` a React component from each page file. But that is not a requirement from vite-pages core.
...
Checkout <Link to="/advanced-fs-routing">the advanced-fs-routing doc</Link> or <Link to="/examples/component-library">"Example: develop a component library"</Link> to learn more about how to create a page with multiple data pieces that originate from multiple files.

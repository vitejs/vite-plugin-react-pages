...
In order to handle the change from from the fileSystem, the "Advanced Filesystem Routing" API has changed from the `findPages` API to the `pageStrategy` API. Checkout <Link to="/advanced-fs-routing">the advanced-fs-routing doc</Link> to learn about the latest API.
